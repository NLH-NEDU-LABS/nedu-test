...
import { ENNEAGRAM_QUESTIONS, calculateEnneagram } from '@/lib/enneagram-scoring';
...
export default function EnneagramQuizClient({ token }: { token: string }) {
  ...
  const handleNext = () => {
    if (currentIdx < ENNEAGRAM_QUESTIONS.length - 1) {
      ...
      // Hoàn thành bài test
      setState('analyzing');
      const type = calculateEnneagram(answers);
  ...
  const handleOptionSelect = (opt: string) => {
    setAnswers(prev => ({ ...prev, [ENNEAGRAM_QUESTIONS[currentIdx].id]: opt }));
  };
    ...
    <div className="max-w-2xl mx-auto p-4 py-10">
      <div className="mb-10">
        <div className="flex justify-between text-sm text-gray-500 mb-2">
          ...
          <span>Câu {currentIdx + 1} / {ENNEAGRAM_QUESTIONS.length}</span>
        </div>
        <div className="h-2 bg-gray-100 rounded-full overflow-hidden">
          <div 
            className="h-full bg-[#8B5E3C] rounded-full transition-all duration-300 ease-in-out" 
            style={{ width: `${((currentIdx + 1) / ENNEAGRAM_QUESTIONS.length) * 100}%` }}
          ></div>
        </div>
      </div>
      ...
      <div className="mt-10 flex justify-between">
        ...
        >
          {currentIdx === ENNEAGRAM_QUESTIONS.length - 1 ? 'Xem kết quả' : 'Tiếp theo'}
        </button>
      </div>
    </div>
  );
