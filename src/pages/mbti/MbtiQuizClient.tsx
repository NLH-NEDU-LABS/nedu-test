...
export default function MbtiQuizClient({ token, nextStep }: { token: string; nextStep?: string }) {
  ...
  const handleNext = () => {
    ...
    setMbtiType(type);
    api
      .post<{ mbti_type: string; mbti_desc?: string }>('/mbti/score', { token, mbti_type: type })
      .then((data) => {
  ...
  return (
    <div className="max-w-2xl mx-auto p-4 py-10">
      ...
      <h2 className="text-2xl font-semibold mb-8 text-gray-800 leading-relaxed">{q.question}</h2>
