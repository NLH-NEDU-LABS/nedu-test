...
export default function EnneagramQuizClient({
  token,
  nextStep,
  mode,
}: {
  ...
  const totalQuestions =
    phase1Questions.length + (center ? ENNEAGRAM_PHASE2[center].length : ENNEAGRAM_PHASE2.gut.length);
  ...
  const handleNext = () => {
    if (phase === 1) {
      if (currentIdx < phase1Questions.length - 1) {
        ...
        setCenter(determineCenter(phase1Answers));
        setPhase(2);
        setCurrentIdx(0);
      }
      return;
  ...
  return (
    <div className="max-w-2xl mx-auto p-4 py-10">
      ...
      <h2 className="text-xl font-medium mb-2 text-gray-800 leading-relaxed">{q.statement}</h2>
      ...
      <div className="space-y-2">
        {LIKERT_KEYS.map((key) => {
          ...
          return (
            ...
            >
              <span
                className={`w-8 h-8 flex-shrink-0 rounded-full flex items-center justify-center text-sm font-semibold border-2 transition-colors ${
                  isSelected ? 'bg-[#8B5E3C] border-[#8B5E3C] text-white' : 'border-gray-200 text-gray-500'
                }`}
              >
                {key}
              </span>
